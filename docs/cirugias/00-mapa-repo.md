...
- **Filtro primer/segundo ojo al crear cirugía**: `src/app/api/search/route.ts` acepta `&cirugia=todos|primer|segundo` → consulta `agenda_cirugias` (excluye `estado='cancelada'`), filtra pacientes sin cirugías previas (primer ojo) o con cirugía previa (segundo ojo) y agrega `ojo_operado` + `cirugias_previas` a cada resultado (con el filtro salta las otras 4 búsquedas de `/api/search`). En `src/app/(dashboard)/cirugias/nueva/page.tsx`: botones Todos/Primer ojo/Segundo ojo en el Paso 1 con etiqueta por paciente (Primer ojo · OD/OI ya operado · ⚠ Ambos ojos) y, al elegir "Segundo ojo", el Paso 3 marca automáticamente el ojo contrario al ya operado (hist. vía `GET /api/cirugias?paciente_id=`); si ambos ojos u OU o sin especificar, aviso y elección manual.
...
- **Reporte de cirugías (B21)**: `GET /api/productividad/reportes/cirugias?desde&hasta&doctor_id` → CSV con las columnas del CSV fuente (`agenda_cirugias` + `pacientes`/`doctores`); en `/productividad` el botón "Cirugías (CSV)" abre `src/components/productividad/ModalRangoFechas.tsx` (mismo modal que "Entradas y salidas" y "Honorarios").
