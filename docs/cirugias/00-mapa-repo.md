...
- Lib: `src/lib/inventario.ts` — `consumirLIO()` (SALIDA_CIRUGIA) y `liberarLIO()` (DEVOLUCIÓN), ya conectados al PATCH de agenda. Seeds: `scripts/seed-inventario-io.{ts,sql}`.
