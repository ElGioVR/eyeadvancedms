...
| ID | Requisito | Estado | Evidencia | Notas |
|---|---|---|---|---|
| PAC-001 | Buscador de paciente por nombre / expediente | CUBIERTO | src/app/(dashboard)/cirugias/nueva/page.tsx: usa `SearchInput` + `fetch('/api/search?q=...&cirugia=...')`; búsqueda por nombre y teléfono/email; filtro **Todos / Primer ojo / Segundo ojo** con etiqueta por paciente y auto-marcado del ojo contrario en el Paso 3 (`/api/search` con `cirugia`, `GET /api/cirugias?paciente_id=`) | Test: `npm run test:b9` → 92 OK. |
...
| LIO-002 | Relación Cirugía → LIO → Inventario por FK, sin copiar información | CUBIERTO | src/lib/lio.ts:validarLIO valida disponibilidad; src/app/api/cirugias/route.ts:22,77 pasa `inventario_item_id` FK al RPC; mig1800000000180:CreateCrearCirugiaRPC.ts:157-168,210 valida LIO e inserta solo `inventario_item_id`; AgendaContent.tsx:handleLIOSelect ya no copia `lio`/`marca_lio`; stock descontado al asignar: `consumirLIO` en POST `/api/agenda`, POST `/api/cirugias` (tras RPC) y PATCH al cambiar `inventario_item_id` (`src/lib/inventario.ts:21-37` idempotente) | La relación es exclusivamente por FK; texto legacy `lio`/`marca_lio` queda vacío al seleccionar desde inventario. Al reemplazar el LIO se libera el anterior (`liberarLIO`). |
...
Ajuste de inventario LIO: el stock se descuenta al **asignar** el LIO a la cirugía (no solo al completar). `consumirLIO` (`src/lib/inventario.ts:14-88`) es idempotente (verifica `SALIDA_CIRUGIA` previa para la misma cirugía+ítem) y se invoca en POST `/api/agenda` (revierte el insert si falla), POST `/api/cirugias` (red de seguridad tras el RPC) y PATCH `/api/agenda/[id]` al cambiar `inventario_item_id` (libera el anterior si se reemplaza). El RPC `crear_cirugia` sigue descontando al crear; la idempotencia evita doble descuento. Validación: `npx tsc --noEmit` → OK; `npx next lint --file` sobre los 4 archivos → OK; `npm run test:b7` → 85 OK; `npm run test:b2` → 33 OK; `npm run test:b4` → 53 OK (fallos preexistentes en b9/b10/b11 ajenos a este cambio).

## Nota posterior — Doctores: alias + vínculo usuario (2026-09-25)

Auditoría y ajuste del flujo usuarios↔doctores: (1) crear un usuario con rol doctor **ya no crea ni vincula un doctor automáticamente** (`src/app/api/configuracion/usuarios/route.ts` POST/PATCH sin lógica de doctores); el vínculo se gestiona solo desde Configuración → Doctores. (2) El vínculo usuario↔doctor acepta usuarios con rol **doctor o administrador**, validado en servidor (`src/app/api/configuracion/doctores/route.ts` POST/PATCH) y en el formulario (`configuracion/doctores/page.tsx` filtra `['doctor','admin']`). (3) `doctores.nombre_completo` pasa a ser **`alias`** (nombre de presentación tipo "DR BAYARDO") con migración `src/migrations/1800000000290-RenameDoctorNombreToAlias.ts` + patch SQL `sql/patch-doctores-alias.sql`, que agrega `nombre`/`apellido` para la identidad real del doctor. Renombre aplicado en ~35 archivos: búsquedas (`src/app/api/search/route.ts`), agenda y su import, cirugías, productividad (rutas, metricas, mis-honorarios, pagos, reportes/cirugias, `src/lib/productividad/liga.ts`, `MotorDevengoService`), dashboard (`src/lib/dashboard-data.ts`, charts), consultas (joins de doctor/estudios) y UI (agenda, cirugías, productividad, ficha de doctores con campos Alias/Nombre/Apellido). El campo API `nombre` de doctores ahora mapea a `alias`. Validación: `npx tsc --noEmit` → OK; `npm run test:b2` → 33 OK; `npm run test:b4` → 53 OK. Requiere aplicar el patch SQL en BD (local o remota por el admin).
