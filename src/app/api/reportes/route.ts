...
export async function GET(request: Request) {
  ...
  const embudoConsultas = {
    ...
    APLAZADA: consultas.filter(c => c.estatus === 'APLAZADA').length,
    REAGENDADA: consultas.filter(c => c.estatus === 'REAGENDADA').length,
    COMPLETADA: consultas.filter(c => c.estatus === 'COMPLETADA').length,
