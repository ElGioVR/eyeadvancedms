...
export async function POST(
  ...
  const { data: liquidacion, error: e1 } = await supabase
    ...
    .select('id, estado, total_devengado, total_ajustes, total_retenciones, neto_pagar, moneda, doctor_id')
    .eq('id', id)
    .maybeSingle();
  ...
  const { error: updateError } = await supabase
    .from('liquidaciones_doctor')
    .update({ estado: 'APROBADA', aprobado_por: auth.user.id, aprobado_at: new Date().toISOString(), updated_at: new Date().toISOString() })
    .eq('id', id);

  if (updateError) return NextResponse.json({ error: 'Error al aprobar' }, { status: 500 });

  await supabase.from('bitacora_honorarios').insert({
    tabla: 'liquidaciones_doctor', registro_id: id, accion: 'APROBACION',
    valor_anterior: { estado: liquidacion.estado }, valor_nuevo: { estado: 'APROBADA' },
