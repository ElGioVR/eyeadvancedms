...
const colaboradoresData: Colaborador[] = [
  {
    id: 1,
    nombre: 'Dra. Irina Rostova',
    email: 'irina.rostova@eyeadvanced.com',
    rol: 'DOCTOR',
    rolColor: 'bg-primary-50 text-primary-700 ring-primary-200',
    estado: 'ACTIVO',
    ultimoAcceso: 'Hoy, 10:15 AM',
    iniciales: 'IR',
    avatarColor: 'bg-primary-500',
  },
  {
    id: 2,
    nombre: 'Dr. Héctor Sánchez',
    email: 'h.sanchez@eyeadvanced.com',
    rol: 'DOCTOR',
    rolColor: 'bg-primary-50 text-primary-700 ring-primary-200',
    estado: 'ACTIVO',
  ...
  {
    id: 3,
    nombre: 'Admin Carlos Mendoza',
    email: 'admin.carlos@eyeadvanced.com',
    rol: 'ADMIN',
    rolColor: 'bg-red-50 text-red-700 ring-red-200',
    estado: 'ACTIVO',
  ...
  },
  {
    ...
    email: 'sofia.recepcion@eyeadvanced.com',
    rol: 'RECEPCIONISTA',
    rolColor: 'bg-amber-50 text-amber-700 ring-amber-200',
    estado: 'ACTIVO',
    ultimoAcceso: 'Hoy, 08:02 AM',
    iniciales: 'SG',
    avatarColor: 'bg-purple-500',
  },
  {
    id: 5,
    nombre: 'Dra. Lucía Ortiz',
    email: 'lucia.ortiz@eyeadvanced.com',
    rol: 'DOCTOR',
    rolColor: 'bg-primary-50 text-primary-700 ring-primary-200',
    estado: 'INACTIVO',
    ultimoAcceso: '15 Ago 2024',
    iniciales: 'LO',
  ...
  },
  {
    ...
    email: 'mateo.mj@eyeadvanced.com',
    rol: 'RECEPCIONISTA',
    rolColor: 'bg-amber-50 text-amber-700 ring-amber-200',
    estado: 'ACTIVO',
    ultimoAcceso: '01 Sep 2024',
    iniciales: 'MR',
...
export default function ConfiguracionPage() {
  ...
  const activeTab = tabs.find((t) => pathname === t.href || pathname.startsWith(t.href + '/'))?.label || 'Usuarios';
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                ...
                {paginated.map((user) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      editingUser?.id === user.id && 'bg-primary-50/40'
                    )}
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={cn(
                          'inline-flex rounded-md px-2.5 py-1 text-xs font-extrabold ring-1 ring-inset',
                          user.rolColor
                        )}
                      >
                        {user.rol}
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={cn(
                          ...
                          user.estado === 'ACTIVO' ? 'text-emerald-600' : 'text-gray-400'
                        )}
                      >
                        <span
                          className={cn(
                            'h-1.5 w-1.5 rounded-full',
                            user.estado === 'ACTIVO' ? 'bg-emerald-500' : 'bg-gray-300'
                          )}
                        />
                        {user.estado}
                      </span>
            ...
            <div className="flex items-center justify-between border-t border-gray-100 bg-gray-50/30 px-6 py-3">
              <span className="text-sm text-gray-400">
                Mostrando {paginated.length} de {filtered.length} usuarios clínicos
              </span>
              <div className="flex items-center gap-1">
                <button
                  onClick={() => setPage(Math.max(1, page - 1))}
                  disabled={page === 1}
                  className="inline-flex items-center gap-1 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                ...
                <div className="flex justify-center">
                  <div
                    className={cn(
                      ...
                      editingUser.avatarColor
                    )}
                  >
                    {editingUser.iniciales}
                  </div>
                </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      className={cn(
                        ...
                        editingUser.estado === 'ACTIVO' ? 'bg-emerald-500' : 'bg-gray-300'
                      )}
                    >
                      <span
                        className={cn(
                          'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition-transform',
                          editingUser.estado === 'ACTIVO' ? 'translate-x-5.5' : 'translate-x-0.5'
                        )}
                      />
                    </button>
                  </div>
